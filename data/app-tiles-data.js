import AppTile from "../models/AppTile";

export const olympiadTiles = [
  new AppTile(
    "imo",
    "Mathematics",
    "Course",
    require("../assets/images/olympiad/imo.webp"),
    "30 min",
    "COURSE",
    "Mohit Kumar Chilkoti",
    "COURSE" // stop using this - category
  ),
  new AppTile(
    "nso",
    "Science",
    "Course",
    require("../assets/images/olympiad/nso.webp"),
    "30 min",
    "COURSE",
    "Sir Issac Newton",
    "COURSE" // stop using this - category
  ),
  new AppTile(
    "ieo",
    "English",
    "Course",
    require("../assets/images/olympiad/ieo.webp"),
    "30 min",
    "COURSE",
    "Pratha Chilkoti",
    "COURSE" // stop using this - category
  ),
  new AppTile(
    "iho",
    "Hindi",
    "Course",
    require("../assets/images/olympiad/iho.webp"),
    "60 min",
    "COURSE",
    "Pandit Ji",
    "COURSE" // stop using this - category
  ),
];

export const myClassrooms = [
  new AppTile(
    "psy",
    "Psychology",
    "Course",
    // "https://www.simplypsychology.org/wp-content/uploads/psychology.jpeg",
    "https://www.bolton.ac.uk/assets/Is-Psychology-a-science-Bolton-University__ResizedImageWzYwMCw0MDld.jpg",
    "20 hr",
    "COURSE",
    "Seema Joshi",
    "COURSE"
  ),
  new AppTile(
    "rsn",
    "Reasoning",
    "Course",
    require("../assets/images/classrooms/reasoning.webp"),
    "30 hr",
    "COURSE",
    "Seema Joshi",
    "COURSE"
  ),
];

export const popularReads = [
  new AppTile(
    "sysd",
    "System Design",
    "Reading",
    "https://cdn.pixabay.com/photo/2024/04/04/10/46/ai-generated-8674827_1280.jpg",
    "30 min",
    "STUDY",
    "Mohit Chilkoti",
    "STUDY"
  ),
];

export const popularQuizzes = [
  new AppTile(
    "dog-quiz-1",
    "Dogs",
    "Quiz",
    "https://cdn.pixabay.com/photo/2021/01/23/07/53/dogs-5941898_1280.jpg",
    "30 min",
    "QUIZ",
    "Mohit Chilkoti",
    "STUDY"
  ),
];

export const storyTiles = [
  new AppTile(
    "akbb1",
    "Akbar Birbal",
    "Story",
    require("../assets/images/stories/akbar-birbal1.webp"),
    "5 min",
    "STUDY",
    "Raja Birbal",
    "STUDY"
  ),
  new AppTile(
    "bgh1",
    "Gautam Buddha - The Enlightened",
    "Bio",
    require("../assets/images/stories/buddha.webp"),
    "7 min",
    "STUDY",
    "Gautam Buddha",
    "STUDY"
  ),
  new AppTile(
    "pct1",
    "Smart Rabbit",
    "Story",
    require("../assets/images/stories/panchatarntar1.webp"),
    "4 min",
    "STUDY",
    "Seema Joshi",
    "Vishnu Sharma",
    "STUDY"
  ),
];

// Let's see if this structure works too
export const bookSummaryTiles = [
  {
    id: "bfg",
    title: "The BFG",
    subtitle: "Book Summary",
    image: require("../assets/images/book-summaries/bfg.webp"),
    duration: "10 min",
    type: "STUDY",
    author: "Seema Joshi",
    category: "STUDY",
  },
  {
    id: "tgwdtm",
    title: "The Girl Who Drank The Moon",
    subtitle: "Book Summary",
    image: require("../assets/images/book-summaries/girlDrankTheMoon.webp"),
    duration: "20 min",
    type: "STUDY",
    author: "Mohit Chilkoti",
    category: "STUDY",
  },
  {
    id: "harry1",
    title: "Harry Potter & The Philosopher's Stone",
    subtitle: "Book Summary",
    image: require("../assets/images/book-summaries/harry-1.webp"),
    duration: "15 min",
    type: "STUDY",
    author: "Pratha Chilkoti",
    category: "STUDY",
  },
  {
    id: "matilda",
    title: "Matilda",
    subtitle: "Book Summary",
    image: require("../assets/images/book-summaries/matilda.webp"),
    duration: "18 min",
    type: "STUDY",
    author: "Pratha Chilkoti",
    category: "STUDY",
  },
];
